...
function pasarUsuarioAPremium(objetoMuchosUsuarios) {
   ...
   for (var i = 0; i < objetoMuchosUsuarios.length; i++) {
      for (var propiedad in objetoMuchosUsuarios[i]) {
         if (propiedad === "esPremium") {
            objetoMuchosUsuarios[i][propiedad] = true;
         }
      }
   }
...
function agregarMetodoCalculoDescuento(objetoProducto) {
   ...
   objetoProducto.calcularPrecioDescuento = function() {
      var total = this.precio * this.porcentajeDeDescuento;
      total = this.precio - total;
      return total;
   }
   return objetoProducto; // Devolver objetoProducto con la propiedad agregada
}
